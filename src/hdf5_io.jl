...
function read_matrix(f::HDF5.Dataset; kwargs...)
  ...
  if haskey(attributes(f), "categories")
    ...
    ordered =
      haskey(attributes(categories), "ordered") && read_attribute(categories, "ordered") == true
    ...
    mat = if ordered
      ...
      PooledArray(
        ...
        Dict{eltype(cats), eltype(mat)}(v => i for (i, v) in enumerate(cats)),
      )
    end
  end
  return mat
...
function write_impl_array(
  ...
  dims::Union{
    ...
    Tuple{Tuple{Vararg{<:Integer, N}}, Tuple{Vararg{<:Integer, N}}},
  },
  compress::UInt8,
) where {N}
